...
router.post("/:cid/product/:pid", async (req, res) => {
  ...
  try {
    const carritoActualizado = await cartM.addProductToCart(
      cartId,
      productId,
      quantity
    );
...
router.delete("/:cid/products/:pid", async (req, res) => {
  ...
  try {
    ...
    const carritoActualizado = await cartM.deleteProductFromCart(
      cartId,
      productId
    );
    ...
    if (!carritoActualizado) {
      return res
        .status(404)
        .json({ error: "No se encontró el carrito o el producto" });
    }
...
router.put("/:cid/products/:pid", async (req, res) => {
  ...
  try {
    const carritoActualizado = await cartM.updateProductQuantity(
      cartId,
      productId,
      nuevaCantidad
    );
