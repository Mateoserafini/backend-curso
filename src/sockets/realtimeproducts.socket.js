...
const socketProducts = (socketServer) => {
  socketServer.on("connection", async (socket) => {
    ...
    socket.on("requestProducts", async () => {
      try {
        const listaDeProductos = await productController.getProductsView();
        console.log("Enviando lista de productos por solicitud:", listaDeProductos);
        socket.emit("enviodeproducts", listaDeProductos);
      } catch (error) {
    ...
    socket.on("addProduct", async (obj) => {
      ...
      try {
        ...
        const listaDeProductos = await productController.getProductsView();
        console.log("Lista de productos actualizada después de añadir:", listaDeProductos);
    ...
    socket.on("deleteProduct", async (id) => {
      ...
      try {
        ...
        const listaDeProductos = await productController.getProductsView();
        console.log("Lista de productos actualizada después de eliminar:", listaDeProductos);
        socketServer.emit("enviodeproducts", listaDeProductos);
      } catch (error) {
    ...
    socket.on("updateProduct", async (updatedProduct) => {
      ...
      try {
        ...
        const updatedResult = await productController.updateProduct(id, productoActualizado);
        console.log("Resultado de la actualización:", updatedResult);

        if (updatedResult) {
          socket.emit("productUpdated", updatedResult);
        } else {
          socket.emit("productUpdateFailed", { message: `Producto con ID ${id} no encontrado` });
        }
      } catch (error) {
        console.error(`Error al procesar la actualización del producto con ID ${updatedProduct.id}:`, error);
